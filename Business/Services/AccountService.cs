...
// Tagit hjälp av chatgpt med servicebus
public class AccountService(UserManager<IdentityUser> userManager, ServiceBusPublisher busPublisher)
    : AccountGrpcService.AccountGrpcServiceBase
{
    private readonly UserManager<IdentityUser> _userManager = userManager;
    private readonly ServiceBusPublisher _busPublisher = busPublisher;
    ...
    public override async Task<CreateAccountReply> CreateAccount(CreateAccountRequest request, ServerCallContext context)
    {
        ...
        if (result.Succeeded)
        {
            ...
            var busMessage = JsonSerializer.Serialize(new
            {
                UserId = user.Id,
                Email = user.Email
            });

            await _busPublisher.SendMessageAsync(busMessage);
        }
